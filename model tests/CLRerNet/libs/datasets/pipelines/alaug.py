...
class Alaug(object):
    ...
    def aug(self, data):
        ...
        aug_input = dict(image=img, keypoints=keypoints_val)
        if masks is not None:
            aug_input['mask'] = masks
        if self.bbox_params is not None and bboxes is not None:
            aug_input['bboxes'] = bboxes
            aug_input['bbox_labels'] = bbox_labels
        elif self.bbox_params is not None:
            aug_input['bboxes'] = []
            aug_input['bbox_labels'] = []
        aug = self.__augmentor(**aug_input)
        data['img'] = aug['image']
