...
class Instances:
    ...
    def denormalize(self, w, h):
        ...
        if not isinstance(self.segments, str):
            self.segments[..., 0] *= w
            self.segments[..., 1] *= h
        if self.keypoints is not None:
            self.keypoints[..., 0] *= w
    ...
    def remove_zero_area_boxes(self):
        ...
        if not all(good):
            ...
            if len(self.segments) and not isinstance(self.segments, str):
    ...
    def concatenate(cls, instances_list: List["Instances"], axis=0) -> "Instances":
        ...
        if not isinstance(instances_list[0].segments, str):
            cat_segments = np.concatenate([b.segments for b in instances_list], axis=axis)
        else:
            cat_segments = 'Placeholder'
