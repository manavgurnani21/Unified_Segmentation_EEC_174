...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        mask4_list = [np.full((s * 2, s * 2), 0, dtype=np.uint8) for _ in labels["img"][1]]
        for i in range(4):
            labels_patch = labels if i == 0 else labels["mix_labels"][i - 1]
            # Load image
            img = labels_patch["img"][0]
            h, w = labels_patch.pop("resized_shape")
            ...
            ###### JW create 4_mask
            for count, mask in enumerate(mask4_list):
                mask[y1a:y2a, x1a:x2a] = labels_patch["img"][1][count][y1b:y2b, x1b:x2b]
            ...
            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = [img4, mask4_list]
        if labels.get('type_task'):  ### JW add type_task to label for identify the task in the following augment.
            final_labels["type_task"] = labels['type_task']
        return final_labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        for count, mask in enumerate(labels["img"][1]):
            labels["img"][1][count] = cv2.warpAffine(mask, M[:2], dsize=self.size, borderValue=(0,0))
        ...
        if len(segments) and not isinstance(segments, str):
            bboxes_seg, segments = self.apply_segments(segments, M)
        segmentation_classes = labels['type_task']['segmentation']  ### JW identify the different task and process each one avoid detection boxes wrong.
        segmentation_indices = np.where(np.isin(cls, segmentation_classes))[0]
        if len(segmentation_indices) > 0 and len(segments) > 0 and not isinstance(segments, str):
            bboxes[segmentation_indices] = bboxes_seg[segmentation_indices]
        ...
        ### JW obtain the i separately because different task has different area_thr
        detection_indices = np.where(~np.isin(cls, segmentation_classes))[0]

        i = np.zeros(len(cls), dtype=bool)

        # segmentation area_thr
        if len(segmentation_indices) > 0:
            i_segmentation = self.box_candidates(
                box1=instances.bboxes[segmentation_indices].T,
                box2=new_instances.bboxes[segmentation_indices].T,
                area_thr=0.01
            )
            i[segmentation_indices] = i_segmentation
        # TODO The sample must have at least one object for detection now. Needs to be adaptive to special cases of zero.
        # detection area_thr
        i_detection = self.box_candidates(
            box1=instances.bboxes[detection_indices].T,
            box2=new_instances.bboxes[detection_indices].T,
            area_thr=0.10
        )
        i[detection_indices] = i_detection

        labels["instances"] = new_instances[i]
        labels["cls"] = cls[i]
        labels["img"] = [img, labels["img"][1]]
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        if self.direction == "vertical" and random.random() < self.p:
            img = np.flipud(img)
            for count, mask in enumerate(labels["img"][1]):
                labels["img"][1][count] = np.ascontiguousarray(np.flipud(mask))
            instances.flipud(h)
        if self.direction == "horizontal" and random.random() < self.p:
            img = np.fliplr(img)
            for count, mask in enumerate(labels["img"][1]):
                labels["img"][1][count] = np.ascontiguousarray(np.fliplr(mask))
            instances.fliplr(w)

            # Sync the CLR lane-curve targets with the horizontal image flip.
            # The lane_seg_mask (labels["img"][1]) is flipped just above, but
            # the 78-D curve vectors are a SEPARATE label key, so without
            # this the cls/xytl/iou lane losses would supervise a flipped
            # image with un-flipped curve targets (silent corruption on ~half
            # the batch). Encoding from P1 vertices_to_clr_vector:
            #   [1]=pos score, [3]=start_x (px, 640 frame), [4]=theta (norm),
            #   [5]=length (strips), [6:]=x-values (px); sentinels = -1e5.
            # Horizontal flip in the W-px frame: x -> (W-1)-x, theta -> 1-theta;
            # start_y / length / slot indexing unchanged. This re-enables the
            # single strongest regularizer (the original RMT-PPAD trains with
            # flip ON; we had it OFF, which is the main overfitting cause).
            lt = labels.get("lane_targets", None)
            if lt is not None and getattr(lt, 'ndim', 0) == 2 and lt.shape[1] >= 7:
                W = img.shape[1]
                arr = lt.clone() if hasattr(lt, 'clone') else lt.copy()
                rows = arr[:, 1] >= 0.5            # real lanes only
                arr[rows, 3] = (W - 1) - arr[rows, 3]      # start_x
                arr[rows, 4] = 1.0 - arr[rows, 4]          # theta
                xv = arr[:, 6:]
                valid = (rows[:, None] if hasattr(rows, 'unsqueeze') else rows[:, None]) & (xv > -1e4)
                xv[valid] = (W - 1) - xv[valid]            # x-values (skip sentinels)
                labels["lane_targets"] = arr

            # For keypoints
            if self.flip_idx is not None and instances.keypoints is not None:
                instances.keypoints = np.ascontiguousarray(instances.keypoints[:, self.flip_idx, :])
        labels["img"] = [np.ascontiguousarray(img), labels["img"][1]]
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        Examples:
            ...
            >>> augmented_labels = augmenter(labels)
        """
        ###### JW due to original Ultralytics didn't use it. I do not modify this augment.
        if self.p == 0:
            return labels
        im = labels["img"]
        cls = labels["cls"]
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        # P8 fix: RMT-PPAD packs `labels["img"] = [img_ndarray, [seg_masks]]`
        # so its mask-aware transforms (RandomPerspective/RandomHSV/RandomFlip)
        # can co-transform masks alongside the image. Albumentations is the
        # one transform that wasn't patched for this packing. Unwrap to
        # operate on the bare ndarray, then re-wrap. Albumentations only
        # applies PHOTOMETRIC ops to the image - the masks stay untouched
        # and that's fine because photometric ops don't need to update them.
        _packed = isinstance(labels["img"], (list, tuple)) and len(labels["img"]) == 2
        img_arr = labels["img"][0] if _packed else labels["img"]
        ...
        if self.contains_spatial:
            ...
            new_img = self.transform(image=img_arr)["image"]
            if _packed:
                labels["img"][0] = new_img
            else:
                labels["img"] = new_img
...
class Format:
    ...
    def __call__(self, labels):
        ...
        """
        img = labels.pop("img")
        # P6/P8: `img` is always `[img_ndarray, [seg_masks]]` from the
        # patched update_labels_info. seg_masks is empty if the dataset
        # has no seg path (legacy behavior wouldn't crash either way).
        img_arr = img[0]
        seg_masks_list = img[1] if isinstance(img, (list, tuple)) and len(img) > 1 else []
        h, w = img_arr.shape[:2]
        cls = labels.pop("cls")
        instances = labels.pop("instances")
        ...
        if self.return_mask:
            if seg_masks_list:
                labels["merge_mask"] = torch.from_numpy(np.stack(seg_masks_list, axis=0))
            else:
                # Empty seg path - emit a (1, H, W) zero tensor so collate_fn's
                # stack still produces a valid shape.
                labels["merge_mask"] = torch.zeros((1, h, w), dtype=torch.uint8)
        labels["img"] = self._format_img(img_arr)
    ...
    def _format_segments(self, instances, cls, w, h, segmentation_cls):
        """
        ...
        Args:
            ...
            segmentation_cls (list): Class of segmentation.
        ...
        if self.mask_overlap:
            ...
            cls = cls[sorted_idx]
        elif self.merge_mask:       ### JW combine the same class object into one mask
            masks = polygons2masks((h, w), segments, color=1, downsample_ratio=self.mask_ratio)
            num_classes = len(segmentation_cls)
            H, W = masks.shape[1], masks.shape[2]

            combined_mask = np.zeros((num_classes, H, W), dtype=masks.dtype)

            for number, class_id in enumerate(segmentation_cls):
                indices = [i for i, c in enumerate(cls) if c[0] == class_id]

                if indices:
                    combined_mask[number] = np.sum(masks[indices], axis=0)
        ...
        return combined_mask, masks, instances, cls
