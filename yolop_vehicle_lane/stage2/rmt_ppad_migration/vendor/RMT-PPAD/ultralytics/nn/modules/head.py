...
from .lane_head import LaneSegHead  # P4: lane-only seg head (GCAtoCLRAdapter + CLRHeadForSquareImage)
from .aux_seg_head import AuxiliaryDenseHeads  # AUX: training-only dense drivable+lane seg
from .lane_bezier_head import LaneBezierHead  # B4: bezier head (optional LCM)
...
class MTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        seg_decoder=None,  # P4: None -> TransformerSegmentationDecoder (original), "clr_lane" -> LaneSegHead
        ablation_mode=None,  # P8: None | "no_square_priors" | "no_gca". Applied only when seg_decoder=="clr_lane".
        hd=256,  # hidden dim
        nq=300,  # num queries
        ndp=4,  # num decoder points
        nh=8,  # num head
        ...
        self.imgsz = 640
        # P4: pick seg head implementation. None / unknown -> original
        # TransformerSegmentationDecoder. "clr_lane" -> lane-only LaneSegHead
        # (drivable area is dropped entirely; ns_classes=1 from the YAML).
        self.seg_decoder_mode = seg_decoder
        # P8 ablation: when seg_decoder == "clr_lane", ablation_mode tweaks
        # how the lane head is constructed/fed.
        #   None / "default"    -> CLRHeadForSquareImage + GCA features
        #   "no_square_priors"  -> base CLRHead (24/144/24 priors)
        #   "no_gca"            -> CLRHeadForSquareImage but seg_head gets raw
        #                          input_proj features (skip task_adapter_seg/gate_seg)
        # B4: bezier_lane recognizes its own ablation_mode set:
        #   None / default              -> cubic Bezier, no LCM, GCA on
        #   "with_lcm"                  -> + LaneComplexityModule
        #   "no_gca"                    -> bypass GCA for the lane path
        #   "with_lcm,no_gca"           -> both
        if seg_decoder in ("clr_lane", "bezier_lane"):
            self.ablation_mode = ablation_mode
        else:
            self.ablation_mode = None
        ablation_tokens = set()
        if isinstance(self.ablation_mode, str):
            ablation_tokens = {t.strip() for t in self.ablation_mode.split(',') if t.strip()}
        self.lane_uses_gca = 'no_gca' not in ablation_tokens
        if seg_decoder == "clr_lane":
            self.seg_head = LaneSegHead(
                hidden_dim=hd, ns_classes=1, img_size=self.imgsz,
                use_square_priors=('no_square_priors' not in ablation_tokens),
            )
        elif seg_decoder == "bezier_lane":
            self.seg_head = LaneBezierHead(
                hidden_dim=hd, ns_classes=1, img_size=self.imgsz,
                use_square_priors=('no_square_priors' not in ablation_tokens),
                use_lcm=('with_lcm' in ablation_tokens),
            )
        else:
        ...
        # AUX dense seg (training-only). Re-attaches the original RMT-PPAD
        # drivable+lane dense decoder onto the shared FPN features to inject
        # dense gradients into the trunk and fight detection gradient
        # domination. Env-driven (USE_AUX_SEG=1) so no YAML edit is needed;
        # the module is CONSTRUCTED ONLY WHEN ON, so an off run keeps the
        # exact same parameters/state_dict as before (and inference never
        # touches it - see forward()).
        import os as _os_aux
        self.use_aux_seg = _os_aux.environ.get('USE_AUX_SEG', '') == '1'
        self._aux_seg_output = None
        if self.use_aux_seg:
            n_aux = int(_os_aux.environ.get('AUX_SEG_CLASSES', '2'))
            self.aux_seg = AuxiliaryDenseHeads(
                hidden_dim=hd, num_classes=n_aux, img_size=self.imgsz,
            )
            print(f'[MTDETRDecoder] AUX dense seg ON: {n_aux}-class '
                  f'(ch0=drivable, ch1=lane), training-only', flush=True)
    ...
    def forward(self, x, batch=None):
        ...
        # P4: LaneSegHead returns a dict; the original TransformerSegmentationDecoder
        # returns (seg_mask_tensor, aux_list_tensor_or_None). Branch on the
        # mode flag so the existing tuple-unpacking path stays untouched.
        if self.seg_decoder_mode in ("clr_lane", "bezier_lane"):
            # Both lane heads return a dict; the trainer's loss function
            # discriminates between them by what's INSIDE the dict (the
            # B5 Bezier loss path looks for 'predictions_lists' tensors
            # of shape (B, P, 16), the P5 polyline path looks for the
            # CLRHead output dict with x-offset arrays).
            seg_mask = self.seg_head(x_proj, imgsz=self.imgsz, batch=batch)
            aux_list = None
        else:
            seg_mask, aux_list = self.seg_head(x_proj, self.imgsz)

        # AUX dense seg routing (Step 3): TRAIN-ONLY. Run the dense aux heads
        # on the SAME shared seg features (x_proj) and stash the (B,C,H,W)
        # logits for the loss to pick up. At eval the branch is never
        # executed - no aux compute, nothing added to the graph, no effect on
        # the returned detection/lane outputs (verified by test_aux_routing).
        if self.training and getattr(self, 'use_aux_seg', False):
    ...
    def _get_encoder_input(self, x):
        ...
        for i, feat in enumerate(x):
            ...
            # Segmentation gate (skipped in P8's no_gca ablation row).
            if getattr(self, 'lane_uses_gca', True):
                ...
                # Bypass GCA: pass raw input-projected features straight to seg_head.
                # task_adapter_seg + gate_seg still exist as ModuleList children
                # (so they show up in named_modules and the optimizer sees them),
                # but they don't influence the seg path's gradients in this row.
                x_proj_seg.append(proj_feat)
                gate_mean_seg_list.append(0.0)
