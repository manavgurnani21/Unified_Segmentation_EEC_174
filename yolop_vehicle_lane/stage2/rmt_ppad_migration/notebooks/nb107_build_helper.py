"""One-shot builder for stage2_notebook_107_laneiou_full_training.ipynb.
...
INTRO = """\
# NB107 - LaneIoU FULL training (Sprint-2 winner: LaneIoU both + aux_seg + gca_floor)
...
ROW = 'laneiou_both_full'
PROJECT = '/content/runs/laneiou_full'
...
print(f'=== laneiou_both_full: LaneIoU(both) + aux_seg ({_da}) + gca_floor on FULL data ===')
print('   ', ' '.join(cmd[3:]), flush=True)
rc = run_streaming(cmd, log_path=os.path.join(LOG_DIR, f'NB107_{ROW}.log'), check=False)
...
RUN = Path('/content/runs/laneiou_full/laneiou_both_full')
...
def build():
    ...
    out = Path(__file__).resolve().parent / 'stage2_notebook_107_laneiou_full_training.ipynb'
