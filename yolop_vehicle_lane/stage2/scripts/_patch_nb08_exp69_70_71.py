"""Append exp69 (CULane pretrain), exp70 (CULane-init joint), exp71 (speed test) to NB08."""
...
EVAL_ENTRIES = [
    ('exp69_rmt_gca_culane_lane_only_pretrain', ['culane8', 'debug', '']),
    ('exp70_rmt_gca_culane_init_anchor_cls_sep_vfl_joint', ['full8_culane_init', 'debug', '']),
    ('exp71_rmt_gca_anchor_cls_sep_vfl_speed_batch32_joint', ['full12_speed', 'debug', '']),
]
PLOT_TAG = {
    'exp69_rmt_gca_culane_lane_only_pretrain': 'culane8',
    'exp70_rmt_gca_culane_init_anchor_cls_sep_vfl_joint': 'full8_culane_init',
    'exp71_rmt_gca_anchor_cls_sep_vfl_speed_batch32_joint': 'full12_speed',
}
...
def _video_entry(stem, run_tag):
    ...
    return f"    ('/content/drive/MyDrive/EcoCAR/training_runs/{stem}_{run_tag}.tar', 'stage2/configs/{stem}.yaml', 'video_profile_{out_tag}'),\n"
...
def patch_eval(text):
    ...
    if anchor is None:
        ...
        anchor = next((c for c in candidates2 if c in text), None)
    if anchor is None:
