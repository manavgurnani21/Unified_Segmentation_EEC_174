...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            # if nl:
            #     merge_mask, masks, instances, cls = self._format_segments(instances, cls, w, h, labels['type_task']['segmentation']) ### JW transfer the segmentation labels into format
            ...
            #     )
            #     merge_mask = np.zeros((self.seg_nc, self.imgsz//self.mask_ratio, self.imgsz//self.mask_ratio))    ### JW add the empty merge_mask, otherwise will get the error. Because some sample do not include any segmentation area of object.
            # labels["masks"] = masks
            labels["merge_mask"] = torch.from_numpy(np.stack(img[1], axis=0))
        labels["img"] = self._format_img(img[0])
