...
class MTDETRPredictor(BasePredictor):
    """
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        seg_result = preds[1][-2]
        mask_thr_tensor = torch.tensor(self.args.mask_threshold, device=seg_result.device).view(1, len(self.args.mask_threshold), 1, 1)
        if mask_thr_tensor.shape[1] != seg_result.shape[1]:
            if mask_thr_tensor.shape[1] == 1:
                mask_thr_tensor = mask_thr_tensor.expand(1, seg_result.shape[1], 1, 1)
            else:
                mask_thr_tensor = mask_thr_tensor[:, :seg_result.shape[1], :, :]
        seg_mask = (torch.sigmoid(seg_result) > mask_thr_tensor).float()
        seg_mask = torch.nn.functional.interpolate(seg_mask, size=(oh, ow), mode='bilinear', align_corners=False)
