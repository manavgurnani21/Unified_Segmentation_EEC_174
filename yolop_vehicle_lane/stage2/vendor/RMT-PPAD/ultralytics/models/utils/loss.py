...
class MTDETRDLoss(DETRLoss):
    ...
    def forward(self, preds, batch, dn_bboxes=None, dn_scores=None, dn_meta=None, seg_mask=None, seg_batch=None):
        ...
        ### JW segmentation loss, generalized for Stage-2 lane-only training.
        ...
        zero = pred_seg.sum() * 0.0

        if pred_seg.shape[1] == 1 or gt_seg.shape[1] == 1:
            lane_pred = pred_seg[:, 0, :, :]
            lane_gt = gt_seg[:, 0, :, :]
            loss_drivable_fl = zero
            loss_drivable_tv = zero
            loss_lane_fl = self.FocalLoss(lane_pred, lane_gt) * self.fl_weight
            loss_lane_tv = self.TL(lane_pred, lane_gt) * self.tversky_weight
        else:
            loss_drivable_fl = self.FocalLoss(pred_seg[:, 0, :, :], gt_seg[:, 0, :, :]) * self.fl_weight
            loss_drivable_tv = self.bce_loss(pred_seg[:, 0, :, :], gt_seg[:, 0, :, :]) * self.tversky_weight
            loss_lane_fl = self.FocalLoss(pred_seg[:, 1, :, :], gt_seg[:, 1, :, :]) * self.fl_weight
            loss_lane_tv = self.TL(pred_seg[:, 1, :, :], gt_seg[:, 1, :, :]) * self.tversky_weight
