...
class DiceLoss(nn.Module):
    """
    Reference by https://www.kaggle.com/code/bigironsphere/loss-function-library-keras-pytorch
    """
    ...
    def forward(self, inputs, targets):
        ...
        dice_loss_per_class = 1 - dice_coeff
        if self.reduction == 'mean':
            dice_loss = dice_loss_per_class.mean()
        elif self.reduction == 'sum':
            dice_loss = dice_loss_per_class.sum()
        else:
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        """
        :param pred: (batch, H, W) - 原始 logits（未经过 Sigmoid）
        :param target: (batch, H, W) - 真实标签，0 是背景，1 是前景
        :return: Dice Loss
        """
        pred = torch.sigmoid(pred)  # 应用 Sigmoid 将 logits 转换为概率
        target = target.float()  # 确保 target 为 float

        intersection = (pred * target).sum(dim=(1, 2))  # 计算交集
        union = pred.sum(dim=(1, 2)) + target.sum(dim=(1, 2))  # 计算并集

        dice_score = (2. * intersection + self.smooth) / (union + self.smooth)
        dice_loss = 1 - dice_score.mean()  # 取 1 - Dice 系数
        return dice_loss
